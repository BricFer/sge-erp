...
const autocompletarInputs = () => {
    ...
    const listaProveedores = document.getElementById("proveedores");

    const ProveedorSeleccionado = listaProveedores.options[listaProveedores.selectedIndex];

    const proveedorData = ProveedorSeleccionado.getAttribute("data-info");

    try {
        const proveedor = JSON.parse(proveedorData);

        document.getElementById("id_proveedor").textContent = proveedor.id || "";
        document.getElementById("cif").textContent = proveedor.cif || "";
        document.getElementById("razon_social").textContent = proveedor.razon_social || "";
        document.getElementById("domicilio").textContent = proveedor.domicilio || "";
        document.getElementById("poblacion").textContent = proveedor.poblacion || "";
        document.getElementById("provincia").textContent = proveedor.provincia || "";
        document.getElementById("cod_postal").textContent = proveedor.cod_postal || "";
...
const completarDetalleFactura = ( selectElement ) => {
    ...
    try {
        ...
        detalleRow.querySelector('.precio_venta').value = producto.precio_compra || "";
